...
public class SuperModel implements Writable {
	...
	public void train(BufferedReader in) throws IOException {
		...
		int l = 0;
		while (line != null && line.length() > 0 && l < 1000) {
			l++;
  ...
  public float[] test(float[] test_records){
    ...
    float[] result = new float[nodes_layer[NUM_LAYER]+nodes_layer[NUM_LAYER-1]];
    for (int j = 0; j < nodes_layer[NUM_LAYER-1]; j++)
      result[j] = sample_mem[NUM_LAYER-1][j];
    for (int j = 0; j < nodes_layer[NUM_LAYER]; j++)
    	result[j+nodes_layer[NUM_LAYER-1]] = sample_mem[NUM_LAYER][j];
    return result;
  }
}
